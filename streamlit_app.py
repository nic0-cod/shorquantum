...
def humanize_years(years):
    if years > 1e9: return f">1 milliard d'années"
    if years > 1e6: return f"{years/1e6:.1f}M d'années"
    if years > 1e3: return f"{years/1e3:.1f}k années"
    if years > 1: return f"{years:.1f} ans"
    if years > 1/12: return f"{years*12:.1f} mois"
...
def plot_periodicity(a, N):
    """Visualise la fonction périodique f(x) = a^x mod N."""
    x = np.arange(1, 21)
    ...
    fig.update_layout(
        ...
        xaxis_title="x (Entrée)", yaxis_title="f(x) (Résultat)",
...
with tabs[1]:
    ...
    with col1:
        ...
        """, unsafe_allow_html=True)
    with col2:
        st.info("💡 Le chiffrement n'est pas une question de magie, mais de **difficulté computationnelle**. Les mathématiques sont faciles dans un sens, mais quasi-impossibles à inverser.")
...
with tabs[3]:
    ...
    st.warning("L'informatique quantique transforme un problème exponentiel en un problème **polynomial**. Ce qui prenait des millénaires ne prend plus que quelques heures.")
...
with tabs[5]:
    ...
    scenario = st.selectbox("Sélectionnez un scénario :", 
